  ...
  try {
    const result = await pool.query(`
      SELECT h.ticker, h.company_name, h.sector, h.asset_class,
             h.quantity, h.avg_cost_basis, h.current_price, h.market_value_usd AS market_value,
             h.unrealized_pnl, h.weight_pct, p.portfolio_name
      FROM holdings h
...
router.get('/report/pdf', requireAuth, requireTable(['holdings', 'portfolios']), async (req, res) => {
  try {
    ...
    const sectors = await pool.query(`
      SELECT sector, SUM(market_value_usd) AS market_value
      FROM holdings GROUP BY sector ORDER BY market_value DESC LIMIT 5
    `);

    const topHoldings = await pool.query(`
      SELECT ticker, company_name, market_value_usd AS market_value,
             weight_pct, unrealized_pnl AS total_pnl
      FROM holdings ORDER BY market_value_usd DESC LIMIT 10
    `);
    ...
    doc.text(`Total Unrealized PnL: $${fmt(k.total_pnl)}`);
