  ...
  try {
    const result = await pool.query(`
      SELECT
        ...
        SUM(h.market_value) FILTER (WHERE h.asset_class = 'Equity') AS equity_aum,
        SUM(h.market_value) FILTER (WHERE h.asset_class = 'Fixed Income') AS fi_aum,
        SUM(h.market_value) FILTER (WHERE h.asset_class = 'Alternatives') AS alt_aum,
  ...
  try {
    const result = await pool.query(`
      SELECT
        ...
        SUM(h.market_value) AS aum,
        SUM(h.unrealized_pnl + h.realized_pnl) AS total_pnl,
        ROUND((SUM(h.unrealized_pnl + h.realized_pnl) / NULLIF(SUM(h.cost_basis * h.quantity), 0) * 100)::numeric, 2) AS return_pct
      FROM portfolios p
      LEFT JOIN holdings h ON p.portfolio_id = h.portfolio_id
      WHERE p.is_active = true
      GROUP BY p.portfolio_id, p.portfolio_name, p.portfolio_type, p.base_currency
